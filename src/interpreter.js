...
class Interpreter {
    ...
    exec(node) {
        switch (node.type) {
            ...
            case 'Debug': console.log('System aura: immaculate'); return;
            case 'Ghost': process.exit(0);
    ...
    execInput(node) {
        // Synchronous user input via child_process (simple cross-platform approach)
        // Prompt is optional display text
        const prompt = node.prompt ? node.prompt + ' ' : '> ';
        process.stdout.write(prompt);
        // Read one line synchronously
