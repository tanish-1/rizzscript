...
class Lexer {
    ...
    error(msg) {
        throw new Error(`Lexer error at line ${this.line}, col ${this.column}: ${msg}`);
    }
    ...
    tokenize() {
        while (this.pos < this.source.length) {
            ...
            if (multi) {
                this.tokens.push({ type: multi.type, value: multi.value, line, col });
                continue;
            }
            ...
            if (/\d/.test(ch) || (ch === '-' && /\d/.test(this.peek(1)))) {
                ...
                this.tokens.push({ type: TokenType.NUMBER, value: num, line, col });
                continue;
            }

            // Two-char operators
            if (ch === '=' && this.peek(1) === '=') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.EQ, value: '==', line, col }); continue; }
            if (ch === '!' && this.peek(1) === '=') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.NEQ, value: '!=', line, col }); continue; }
            if (ch === '<' && this.peek(1) === '=') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.LTE, value: '<=', line, col }); continue; }
            if (ch === '>' && this.peek(1) === '=') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.GTE, value: '>=', line, col }); continue; }
            if (ch === '&' && this.peek(1) === '&') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.AND, value: '&&', line, col }); continue; }
            if (ch === '|' && this.peek(1) === '|') { this.advance(); this.advance(); this.tokens.push({ type: TokenType.OR, value: '||', line, col }); continue; }
            ...
            if (singles[ch]) {
                this.advance();
                this.tokens.push({ type: singles[ch], value: ch, line, col });
                continue;
            }
            ...
            if (/[a-zA-Z_]/.test(ch)) {
                ...
                if (kwType) {
                    this.tokens.push({ type: kwType, value: id, line, col });
                } else {
                    this.tokens.push({ type: TokenType.IDENT, value: id, line, col });
                }
                continue;
            }
        ...
        this.tokens.push({ type: TokenType.EOF, value: null, line: this.line, col: this.column });
        return this.tokens;
    }
}
